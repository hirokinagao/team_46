...
class AdminController extends Controller
{
    ...
    public function admin(Request $request)
    {
        ...
        //DBと照合してデータ検索（ userテーブルの情報を全てとってくる（社員一覧に必要なデータ））
        $users_list = DB::table('users')->get();  // ⇒ (各画面ごとに、この行のコードは変更となる)
        
        $view = view('admin', [
            'users_list' => $users_list,    //上記でDBから取得したデータ ⇒ (各画面ごとに、この行のコードを変更して使う)
    ...
    public function search(Request $request) 
    {
        ...
        //                         検索結果は同じbladeに表示するため、bladeで表示する為に変数名は上記のadminメソッドと同じに揃えないといけない
        $users_list = DB::table('users')->where('work_id', 'like', "%$data%")->orWhere('name', 'like', "%$data%")->get();

        //検索結果をリダイレクト
        $view = view('admin', [
            'users_list' => $users_list,   //上記でDBから取得したデータ
